...
#### **VM虚拟机存储不少**
宿主机查看虚拟机的存储超过了df -h查看的存储，虚拟机里清空了还不少宿主机占用
```
1、删除快照


2、把磁盘剩余空间全部写成 0，好让宿主机后面做 压缩/瘦身 时能把这些零块回收掉
虚拟机中：
su root
...
关机，宿主机中：
GUI 按钮：虚拟机设置 → 硬盘 → “压缩磁盘以回收未使用的空间”
等同
vmware-vdiskmanager –k xxx.vmdk
如：
"D:\Program Files (x86)\VMware\VMware Workstation\vmware-vdiskmanager" -k kali-linux-2023.1-vmware-amd64.vmdk
```