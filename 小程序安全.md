...
^
## **微信公众号网页的登录**
可能会使用微信一键登录，这个时候用户同意获取logincode，并通过logincode一键登录。

如：www.wx.com/admin/login.html?logincode=o5lLuwxrfjcHpfWCyB-EzFyKwTaa

漏洞存在于后端可能会存储每一个用户的logincode，logincode泄露即可被接管登录。
