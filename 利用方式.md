部分标准反序列化可以直接使用CC、CB链入口类的payload进行可控反射代码命令执行。如：原生、shrio、jdbc反序列化。

部分非标准反序列化的利用链则可以通过某些类(JdbcRowSetImpl)触发JNDI注入去加载恶意类代码执行。如：fastjson反序列化、log4j2注入。
还有是利用某些类(ScriptEngineManager)可以控制远程类加载器，提供exp.jar的SPI服务，http或ftp远程加载jar直接执行jar代码。如：SnakeYaml反序列化。

当然也可能结合利用链打，如fastjson反序列化打jdbc。
当然也可能不出网、需要绕过等。
