...
比如
我们选择 LD_PRELOAD 模式并点击开始按钮，
成功后蚁剑会在 /var/www/html 目录里上传一个.antproxy.php(或其他，见提示弹窗信息) 文件。
我们创建副本, 并将连接的 URL shell 脚本名字改为 .antproxy.php(连接密码不变)获得一个新的shell，
在这个新shell里面就可以成功执行命令了。

比如
选择PHP7_GC_UAF模式，直接就可以执行命令了。

比如
直接返回一个新的shell黑窗口就可以执行命令
```
...
^
## **数据库管理**
右键数据管理。利用http通道实现的数据库管理，方便连接本地用户。mysql不行用mysqli。